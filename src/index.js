import {blackJackDealer,blackJackPlayer} from './Player';
...
while (input === 'Yes') {
    console.log('|');
    prompt('| Hit(Enter) if you are ready to begin');
    console.log('|');
    let bet = Number(prompt('| What is your initial bet: '));
    //Conditional Checks to Make sure that bet is not negative, the player has enough 
    //funds, and is not a decimal
    while (bet < 1 && bet > human.fund || !Number.isInteger(bet)) {
        console.log('|');
        console.log(`| Please input an integer value minimum (1) ma (${human.funds})\n|`);
    ...
    //setting the variable equal to true allows the computer to differentiate getting Black Jack 
    //and having a normal hand of 21 later on
    let blackJack;
    ...
    //Allows the user to keep hitting while their hand value is less than 21 or until 
    ...
    while (human.handValue < 21 && input !== 'Stand') {
        ...
        while (input !== 'Hit' && input !== 'Stand') {
            console.log('| Please input "Hit" or "Stand"\n|');
            input = prompt('| Would you like to Hit or Stand: ');
            console.log('|');
        }
        if (input === 'Hit') {
    ...
    if (human.handValue > 21) {
        ...
        console.log('|');
        //Dealer must hit when hand value is less than 17 and must stand when it is 17
        // or over
        ...
        dealer.showOverview();
        dealer.showHand();
        //Decides the outcome of the round and how the players funds need to be altered 
        //acordingly
        if (dealer.handValue > 21) {
            console.log(`|\n| Congratulations ${human.name}! The Dealer has bust and you have won $${bet}!\n|`);
            human.addFunds(bet * 2);
        } else if (dealer.handValue < human.handValue) {
            console.log(`|\n| Congratulations ${human.name}! Your hand value is closer to 21 
                then the dealer! You have won $${bet}!|\n|`);
            human.addFunds(bet * 2);
        } else if (dealer.handValue > human.handValue) {
            console.log(`|\n| The Dealer's hand value is closer to 21 than your's! You have lost 
                $${bet}!|\n|`);
                // human.removeFunds(bet * 2);
        } else {
            console.log(`│\n│  Close ${human.name}! You and the dealer have tied! Your bet 
                of $${bet} has been returned to you!\n│`);
            human.addFunds(bet);
        }
    }
    console.log(`| Your current balance is $${human.funds}\n|`);

    //Prompt allowing the user whether or not they would like to continue 
    //playing Black Jack and start a new round
