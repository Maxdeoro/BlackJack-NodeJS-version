//Parent class Player that has properties that the children classes blackJackPlayer 
//and blackJackDealer will inherit
//Has three properties (name, hand, and handValue)

import {Card, blackJackDeck} from './Card';
...
class Player {
    ...
    //Uses loops and the .show() property of the card class to log the cards 
    //side by side to the console
    showHand() {
        for (let i = 0; i < 7; i++) {
            let line = '';
...
class blackJackDealer extends Player {
    constructor(name) {
        super(name);
        this._deck = this.blackJackDeck.slice(0);
    };
...
export default {blackJackDealer, blackJackPlayer};
